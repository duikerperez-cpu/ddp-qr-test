...
export default function Page() {
  ...
  async function onEmpresaChange(id:string){
    ...
    setProductosFiltrados(data && data.length ? data : [])
  }
  ...
  async function generarIndividuales(lote:any, cantidad:number){
    ...
    const lista = Array.from({length: cantidad}, (_, i)=>{
      ...
      return {
        ...
        codigo_qr: id,
        url_dpp: `${APP_URL}/dpp/${id}`,
        estado: "activo"
      }
    })
    const {error}=await supabase.from("productos_individuales").insert(lista)
    if(error) alert("Lote creado pero fallo individuales: "+error.message + "\nCorre el SQL de productos_individuales que te di")
    else alert(`Lote ${lote.codigo} creado con ${cantidad} QRs!`)
  }
  ...
  return (
    <div style={{display:"flex", minHeight:"100vh", background:"#0a0a0a", color:"white"}}>
      <div style={{width:220, background:"#121212", borderRight:"1px solid #1e1e1e"}}>
        <div style={{padding:"18px 16px", borderBottom:"1px solid #1e1e1e"}}><div style={{fontWeight:900, fontSize:20}}>VINCULA<span style={{color:"#ff6a00"}}>B</span></div><div style={{fontSize:8, color:"#555"}}>DIGITAL PRODUCT IDENTITY</div></div>
      ...
      <div style={{flex:1, display:"flex", flexDirection:"column"}}>
        <div style={{height:52, background:"#111113", borderBottom:"1px solid #1e1e1e", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 24px"}}>
          ...
          <div style={{fontWeight:700, fontSize:11}}>Administrador</div>
        </div>
        ...
        <div style={{padding:28, flex:1, background:"#080808"}}>
          ...
          {tab==="lotes" && (
            <>
              <div style={{display:"flex", justifyContent:"space-between"}}><h1>Lotes ({lotes.length})</h1><button onClick={()=>{genCodigo(); setShowLoteModal(true)}} style={{background:"#ff6a00", border:"none", color:"white", padding:"10px 18px", borderRadius:6, fontWeight:700, fontSize:12}}>+ Nuevo Lote</button></div>
              <div style={{marginTop:20, display:"flex", flexDirection:"column", gap:10}}>
                {lotes.map((l:any)=>(
                  <div key={l.id} style={{background:"#151518", border:"1px solid #222", borderLeft:"2px solid #ff6a00", padding:14, borderRadius:8, display:"flex", justifyContent:"space-between"}}>
                    <div><div style={{fontWeight:700, fontSize:13}}>{l.codigo} - {l.cantidad} unidades</div><div style={{color:"#666", fontSize:11}}>{getNombre(empresas.find(e=>e.id===l.empresa_id)||{})} • {new Date(l.created_at||Date.now()).toLocaleDateString()}</div></div>
                    <button onClick={()=>setTab("individuales")} style={{fontSize:11, background:"#222", color:"#ff6a00", border:"1px solid #333", padding:"6px 10px", borderRadius:6}}>Ver QRs →</button>
                  </div>
                ))}
              </div>
            </>
          ...
          {tab==="individuales" && (
            <>
              <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}><h1>Productos Individuales ({individuales.length})</h1><button onClick={()=>setTab("lotes")} style={{background:"#ff6a00", border:"none", color:"white", padding:"8px 14px", borderRadius:6, fontSize:11}}>+ Nuevo Lote</button></div>
              <div style={{marginTop:20, display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:12}}>
                {individuales.map((ind:any)=>(
                  <div key={ind.id} style={{background:"#151518", border:"1px solid #222", borderRadius:10, padding:12, textAlign:"center"}}>
                    ...
                    <div style={{marginTop:8, fontSize:10, color:"#555"}}>Lote: {ind.lote_id?.slice(0,18)}</div>
                  </div>
                ))}
                {individuales.length===0 && <div style={{gridColumn:"1 / -1", background:"#151518", border:"1px dashed #333", padding:24, borderRadius:8, textAlign:"center", color:"#666"}}>No hay productos individuales. Crea un lote de 2 unidades y se generarán automático.</div>}
              </div>
            </>
          )}

          {tab==="empresas" && <><h2>Empresas</h2><div style={{marginTop:12}}>{empresas.map((e:any)=><div key={e.id} style={{background:"#151518", border:"1px solid #222", padding:12, borderRadius:6, marginBottom:8}}>{getNombre(e)}</div>)}</div></>}
          {tab==="modelos" && <><h2>Modelos ({modelos.length})</h2><div style={{marginTop:12}}>{modelos.map((m:any)=><div key={m.id} style={{background:"#151518", border:"1px solid #222", padding:12, borderRadius:6, marginBottom:8}}>{m.nombre}</div>)}</div></>}
          {tab==="productos" && <><h2>Productos ({productos.length})</h2><div style={{marginTop:12}}>{productos.map((p:any)=><div key={p.id} style={{background:"#151518", border:"1px solid #222", padding:12, borderRadius:6, marginBottom:8}}>{p.nombre||p.name}</div>)}</div></>}
        </div>
      </div>
      ...
      {showLoteModal && (
        <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:99}}>
          <div style={{background:"#1a1a1e", border:"1px solid #333", borderRadius:12, padding:22, width:460}}>
            <h3 style={{fontWeight:700, marginBottom:6}}>Nuevo Lote</h3><p style={{fontSize:11, color:"#666", marginBottom:16}}>Generará QR automáticamente por cada unidad</p>
